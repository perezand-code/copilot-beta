...
public class WeightedDirectedGraph<V> extends DirectedGraph<V> {
    ...
    public void subtractEdgeWeight(Edge<V> edge, Weight amount) {
        Weight currentWeight = weights.getOrDefault(edge.from(), Collections.emptyMap()).get(edge.to());
        if (currentWeight != null) {
            Weight newWeight = currentWeight.minus(amount);
            if (newWeight.asInt() <= 0) {
                weights.get(edge.from()).remove(edge.to());
            } else {
                weights.get(edge.from()).put(edge.to(), newWeight);
            }
        }
    }
    ...
    public WeightedDirectedGraph<V> copy() {
        ...
        for (V from : weights.keySet()) {
            for (Map.Entry<V, Weight> entry : weights.get(from).entrySet()) {
                copy.addEdge(from, entry.getKey(), entry.getValue());
            }
        }
        return copy;
    }
